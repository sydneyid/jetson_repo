...
def main():
    ...
    # For star11 with 11 lines, need balanced approach:
    # - Lenient enough to detect all 11 lines (lower minimum_point_number helps)
    # - But threshold should be reasonable to avoid assigning too many points to lines
    # Use moderate threshold - lenient for detection, but not too lenient for assignment
    threshold_from_range = data_range * 0.012  # 1.2% of range (moderate - allows detection but creates outliers)
    threshold_from_noise = estimated_noise * 3.0  # 3.0× noise estimate (moderate)
    ...
    final_threshold = min(final_threshold, data_range * 0.04)   # At most 4% of range
    ...
    # For star11 dataset with 11 lines, each line will have fewer points
    # Use very low threshold to allow detection of all 11 lines
    minimum_point_number = max(2, int(len(points) * 0.004))  # 0.4% of points, at least 2 (extremely lenient for 11 lines)
    ...
    try:
        ...
        lines, labeling = pyprogressivex.findLines(
            ...
            threshold=final_threshold,  # Moderate threshold - allows detection but creates outliers
            conf=0.90,  # Lower confidence = more lenient, allows detection of weaker lines
            ...
            max_iters=5000,  # Many more iterations to find all 11 lines
            minimum_point_number=minimum_point_number,  # Very low (0.4% of total, at least 2) for 11 lines
            maximum_model_number=20,  # Allow up to 20 models to ensure we can find all 11
    ...
    # Additional visualization: Lines and outliers only (no inlier points)
    print("\n6. Creating lines-only visualization...")
        ...
        if abs(b) > 1e-10:
            ...
            if np.any(valid):
                ax2.plot(x_line[valid], y_line[valid], '-', 
                       color=detected_colors[idx], linewidth=2.5, alpha=0.9, zorder=2,
                       label=f'Line {idx+1}')
        elif abs(a) > 1e-10:
            # Vertical line
            y_line = np.linspace(y_lims[0], y_lims[1], 200)
    ...
    # Plot outliers only (no inlier points)
    if use_zero_indexed:
        outlier_mask = (labeling == model_number)
    else:
        outlier_mask = (labeling == 0)
    ...
    if np.any(outlier_mask):
        points_outliers = points[outlier_mask]
        ax2.scatter(points_outliers[:, 0], points_outliers[:, 1], 
                  c='red', s=25, alpha=0.6, marker='x', 
                  label=f'Outliers ({np.sum(outlier_mask)} pts)', zorder=3, linewidths=1.5)
    
    ax2.set_xlim(x_lims)
    ax2.set_ylim(y_lims)
    ax2.set_title(f'Detected Lines and Outliers Only ({model_number} lines)', 
                fontsize=12, fontweight='bold')
    ax2.set_xlabel('X')
    ax2.set_ylabel('Y')
    ...
    plt.tight_layout()
    output_file2 = 'star_dataset_lines_only.png'
