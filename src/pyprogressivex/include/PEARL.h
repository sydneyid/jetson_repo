...
{ 
		...
		std::vector<progx::Model<_ModelEstimator>> *models_)
	{
		...
		// SPEED OPTIMIZATION: Early termination if energy improvement is minimal
		const double MIN_ENERGY_IMPROVEMENT = 0.01; // Stop if energy improvement < 1%
		size_t iterations_without_improvement = 0;
		const size_t MAX_ITERATIONS_WITHOUT_IMPROVEMENT = 3; // Stop after 3 iterations without improvement
			...
			iteration_number++ < maximum_iteration_number) // Break when the maximum iteration limit has been exceeded.
		{
			...
			{
				double energy_improvement = abs(previous_energy - energy) / std::max(previous_energy, 1e-10);
				if (energy_improvement < MIN_ENERGY_IMPROVEMENT)
				...
				{
					if (do_logging)
						std::cout << "[Optimization] Early termination: energy improvement < " << MIN_ENERGY_IMPROVEMENT * 100 << "% for " << MAX_ITERATIONS_WITHOUT_IMPROVEMENT << " iterations.\n";
					convergenve = true;
				}
			}
			...
			double &energy_)
	{
		...
		// SPEED OPTIMIZATION: Limit neighbors per point to reduce graph complexity
		// Most spatial coherence is captured by nearest neighbors, so limiting reduces computation
		const size_t MAX_NEIGHBORS_PER_POINT = 8; // Limit to 8 nearest neighbors per point
		if (spatial_coherence_weight > 0.0)
		{
			for (size_t point_idx = 0; point_idx < point_number; ++point_idx)
			{
				const auto &neighbors = neighborhood_graph_->getNeighbors(point_idx);
				size_t neighbor_count = 0;
				for (const size_t &neighbor_idx : neighbors)
				{
					if (point_idx != neighbor_idx && neighbor_count < MAX_NEIGHBORS_PER_POINT)
					{
		...
		// SPEED OPTIMIZATION: Limit alpha-expansion cycles (default 1000 -> 50)
		// Most labelings converge in <10 cycles, so 50 is sufficient for speed
